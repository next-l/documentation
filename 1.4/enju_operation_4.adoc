...
登録した画像が不要になった場合など、書誌情報の編集画面から削除することができます。

. 登録済みで削除したい画像がつけられた書誌情報の編集画面を開き、右メニューのなかの「画像ファイルの一覧」をクリックします。
+
image::../assets/images/1.4/image_operation_edit.png[書誌情報編集画面の中の「画像ファイルの一覧」リンク]
+
