...
[NOTE]
====
この処理をすると発注リストの状態が「発注済み」に変更されます。
...
[NOTE]
====
[既定の本棚]は、TSVファイルに[本棚]を指定しない場合やTSVファイルで不正な本棚を指定していた場合に登録される本棚となります。TSVファイルに正しい[本棚]が指定されていた場合はTSVファイルを優先します。
====
...
. [[show-import-status]]今までのインポート結果の一覧が表示されます。[状態]を見ると処理結果がわかります。
+
...
[NOTE]
====
この画面は[資料の受入]メニュー->[TSVファイルからのインポート]->
...
. 個々のレコードの登録の成否を確認します。 やり方は <<show-import-status,「TSVファイルを読み込んで登録する」 の 6.>> 以降の手順と同様です。
...
[NOTE]
====
過去のISBNレコードの登録の成否を確認する方法を紹介します。
...
[NOTE]
====
著者については、入力し始めると、すでに登録されている著者の候補が出てくるので、同じものがあれば、そのメニューから選んで決定することができます。
...
[NOTE]
====
ここで、本のタイトルのリンクをたどり、その先の資料の表示画面の右メニュー[編集]をクリックすることでも編集画面に移動します。
...
[NOTE]
====
Enjuで利用できる使える役割については、「著作の関係」機能の「名称」に一覧されています。
「著作の関係」は、下図の通り、システムの設定メニューからリンクされています。

image::../assets/images/1.4/operation-configuration-create_types.png[システムの設定画面における「著作の関係」画面へのリンク]

image::../assets/images/1.4/operation-create_types-list.png[「著作の関係」の一覧例]
====
...
件名標目の種類については、xref:enju_setup_4#setup-subject-heading-type[初期設定マニュアル「件名標目の種類を設定する」]を参照してください 。
...
分類の種類については、xref:enju_setup_4#setup-classification-type[初期設定マニュアル「分類の種類を設定する」]を参照してください。
...
[[required_role]]
===== manifestation_required_role, item_required_role

書誌情報や所蔵情報を参照可能な権限を設定します。
...
例えば、作業中の書誌・所蔵情報などで一時的に一般利用者からは見えないレコードとしたい場合に、`Librarian` または `Administrator` の参照権限を割り当てることで実現できます。

[NOTE]
====
書誌情報に対しては `manifestation_required_role`、所蔵情報に対しては `item_required_role` というフィールド名になります。	
====
...
. [[input-value]]図書館や貸出状態などを設定して、［登録する］ボタンをクリックします。
+
...
[NOTE]
====
「貸出中」や「取り置き中」などのように除籍してはいけない状態だった場合、「更新する」をクリック後に次のようなエラーが表示されます（以下の例は予約されている資料の場合のエラーです）。エラーが出た場合は、資料の状態を除籍ができる処理をしてください（貸出中なら返却処理をするなど）。
...
. <<update-item-info,「すでに登録された所蔵情報を修正登録する」>>の1～4を実行します。
+
...
[NOTE]
====
[連続除籍の一覧]をクリックすると、連続除籍機能で除籍した資料のみの一覧が閲覧できます。
====
...
Enjuでは、資料ごとの本文テキストやスキャン画像など、別ファイルを登録して添付ファイルとすることができます。

[NOTE]
====
添付ファイルは、各資料ごとに一つだけ付けることができ、複数のファイルを添付することはできませんので、ご注意ください。
====

==== 添付ファイルの登録

. ファイルを添付したい資料の書誌情報を開き、その詳細画面から編集画面を開きます。
+
image::../assets/images/1.4/attachment_edit.png[書誌情報の編集画面を開く]
+
. 編集画面フォームの中ほどにある「ファイル」項目から「ファイルを選択」をクリックしてファイルを追加します。
+
image::../assets/images/1.4/attachment_file_add.png[添付ファイルを追加する]
+
. ファイル選択時には、ファイル選択ウィンドウから添付したいローカルファイルを選びます。
+
image::../assets/images/1.4/attachment_file_select.png[添付ファイルを選択する]
+
. 書誌編集フォームの添付ファイル項目にて、選択されたローカルファイルが追加されていることを確認し、フォーム末尾の「更新する」をクリックします。
+
image::../assets/images/1.4/attachment_file_added.png[添付ファイルの追加を確認する]
+
. 更新された書誌詳細画面にて、添付ファイルへのリンクが付いていることを確認します。
+
image::../assets/images/1.4/attachment_file_manifestation.png[添付ファイルへのリンクを確認する]
...
image::../assets/images/1.4/attachment_edit.png[書誌情報の編集画面を開く]
+
. 編集画面フォームの中ほどにある「ファイル」項目の直下にある「ファイルを削除」項目から、削除したいファイルの「削除」チェックボックスをクリックして選択します。
+
image::../assets/images/1.4/attachment_file_delete.png[削除したい添付ファイルを選択する]
+
. 正しく選択されていることを確認して、書誌編集フォーム末尾の「資料を更新する」をクリックします。
+
. 更新された書誌詳細画面にて、添付ファイルへのリンクが削除されていることを確認します。
+
image::../assets/images/1.4/attachment_file_manifestation_deleted.png[添付ファイルへのリンクが削除されていることを確認する]
...
[NOTE]
====
右メニューの[編集]をクリックすると編集の画面に遷移します。[画像ファイルの一覧]をクリックすると画像ファイルの一覧に遷移します。
====
...
登録した画像が不要になった場合など、書誌情報の詳細画面から削除することができます。

. 登録済みで削除したい画像がつけられた書誌情報の詳細画面を開き、右メニューのなかの「画像ファイルの一覧」をクリックします。
+
image::../assets/images/1.4/image_operation_edit.png[書誌情報詳細画面の中の「画像ファイルの一覧」リンク]
+
. 当該書誌情報に関連付けられた画像ファイルの一覧が出るので、そのなかから、削除したい画像の「削除」リンクをクリックします。
+
image::../assets/images/1.4/image_operation_delete.png[画像ファイルの一覧画面]
+
. 本当に削除してもよいか確認する確認画面が出るので、削除してもよい場合は「OK」ボタンをクリックします。
+
image::../assets/images/1.4/image_operation_delete_confirm.png[画像ファイルの削除確認画面]
+
. 削除に成功すると、削除しましたというメッセージが出ることを確認します。
+
image::../assets/images/1.4/image_operation_deleted.png[画像ファイルの削除に成功しました]
...
[NOTE]
====
設定内容を変更する場合は［編集］を、削除する場合は［削除］をクリックします、詳細を見たい場合は[表示]をクリックします。[添付先]のリンクをたどれば、添付した資料の「資料の表示」画面にいきます。[ファイル名]をクリックすると画像ファイルが閲覧できます。右メニューにはそれぞれの一覧へ遷移するリンクがあります。
====
...
[開始]のものは現在、実行中です。エクスポート完了時には xref:enju_operation_2#message-function[メッセージ機能] にてお知らせします（「エクスポートが完了しました」というメッセージが送られてきます）。xref:enju_user_3#profile-info[アカウントにメールアドレスの設定（利用者マニュアル「メールアドレスや名前などを変更する」参照）]をしておき、メールサーバーの設定がされていれば、メールでもお知らせが来ます。
+
image::../assets/images/1.1/image_operation_export_bib_5.png[エクスポート結果の一覧表示]
+
[NOTE]
====
この画面は[図書館の管理]メニュー->[エクスポート]->[書誌と所蔵のエクスポート]とすればいつでも閲覧できます。
====
...
[NOTE]
====
[削除]とするとこのエクスポートファイルを削除することができます。
====

[[bib-tsv-export]]
==== 【Column】TSVファイルで出力される項目

xref:enju_webapi_1.adoc#bib-tsv-export[Web APIマニュアル「書誌と所蔵のTSVの項目」]を参照してください。
