...
+
image::1.1/image_operation_242.png[キーワードを入力]
+
[NOTE]
====
...
+
image::1.1/image_operation_244.png[該当する資料が検索される]

. [[show-item]]資料の詳細が表示されます。
+
image::1.3/image_operation_244_2.png[該当する資料の表示]
...
+
image::1.1/image_operation_245.png[（条件を詳しく指定して検索）クリック]

. 複数の条件を指定して［検索］ボタンをクリックします。  
+
image::1.2/advanced_search_1.png[複数の条件を指定]
+
[NOTE]
====
...
. 検索結果を表示する。
+
image::1.1/search_result_facet_1.png[検索結果を表示]

. 出版日の項目から「2010 - 2019」と書かれたリンクをクリックします。
+
image::1.1/search_result_facet_2.png[出版年を使って絞り込む]
+
[NOTE]
====
...
+
image::1.1/search_result_facet_3.png[出版年 2010 - 2019 での絞り込み結果]
...
+
image::1.2/search_result_sort_1.png[ソート方法を選ぶ]

. 選んだソート方法の順序で表示されます。
+
image::1.2/search_result_sort_2.png[ソート結果]
...
+
image::1.2/search_result_number_1.png[表示件数を選ぶ]

. 選んだ表示件数分の検索結果が表示されます。
+
image::1.2/search_result_number_2.png[検索結果表示]

=== 検索結果のフィード(RSS)を取得する

検索結果のRSSを取得することができます。
検索結果で表示されている一覧冒頭の結果をそのまま出力しますので、検索結果の表示件数設定が「10件ずつ表示」であれば最初の10件のみを表示します。
検索結果の表示件数設定が「20件ずつ表示」であれば最初の20件のみを表示します。

[NOTE]
====
検索結果が複数の画面にわたる場合でも、どの画面の[検索結果のフィード]でもかならず一覧先頭の内容のみとなります。
====

. 右メニューの[検索結果のフィード]をクリックします。
+
image::1.4/search_result_rss_1.png[検索結果のフィード]

. 最近登録された10件のRSSが表示されます。
+
image::1.4/search_result_rss_2.png[RSS形式の出力結果例]
...
. 書き出しをしたい検索結果画面を表示させます。[XLSX]リンクをクリックします
+
image::1.4/search_result_xlsx.png[XLSX形式で書き出し]
+
[NOTE]
====
...
* 出力されるTSVの例：xref:enju_webapi_1.adoc#example-bib-xlsx[「書誌と所蔵のXLSXの例」（Web APIマニュアル）]
* 出力される項目の詳細：xref:enju_webapi_1.adoc#bib-tsv-export[「書誌と所蔵のTSVの項目」（Web APIマニュアル）と同一]
...
+
image::1.2/search_result_tsv_1.png[TSV]
+
[NOTE]
====
現在、画面でみえている結果の分だけが書き出しができます。
...
* 出力されるTSVの例：xref:enju_webapi_1.adoc#example-bib-tsv[「書誌と所蔵のTSVの例」（Web APIマニュアル）]
* 出力される項目の詳細：xref:enju_webapi_1.adoc#bib-tsv-export[「書誌と所蔵のTSVの項目」（Web APIマニュアル）]
...
+
image::1.1/search_result_rdf_1.png[RDF/XML]
+
[NOTE]
====
現在、画面でみえている結果の分だけが書き出しができます。
...
* xref:enju_webapi_1.adoc#get-rdf-xml[Web APIマニュアル「RDF/XML形式のデータを取得する」]参照
...
+
image::1.1/search_result_mods_1.png[MODS]
+
[NOTE]
====
現在、画面でみえている結果の分だけが書き出しができます。
...
+
image::1.3/search_result_json_1.png[TSV]
+
[NOTE]
====
現在、画面でみえている結果の分だけが書き出しができます。
...
+
* 出力されるJSONの例：xref:enju_webapi_1.adoc#example-bib-json[「書誌と所蔵のJSON形式のデータの例」（Web APIマニュアル）]
* 出力される項目の詳細：xref:enju_webapi_1.adoc#bib-json-export[「書誌と所蔵のJSON形式の項目」（Web APIマニュアル）]

=== 検索結果詳細の書き出し

[[bib-rdf-xml-one-record]]
==== RDF/XML 形式で書き出し（詳細）

. 書き出しをしたい検索結果詳細画面を表示させます。[RDF/XML]リンクをクリックします
+
image::1.1/search_result_full_rdf_1.png[RDF/XML]

. RDF/XML 形式のファイルがダウンロードできます。

* 出力されるRDF/XMLの例：xref:enju_webapi_2#example-bib-rdf-xml[「書誌のRDF/XML形式のデータの例」（Web APIマニュアル参照）]
...
+
image::1.1/search_result_full_mods_1.png[MODS]
...
+
image::1.1/search_result_full_tsv_1.png[TSV]

. TSV 形式のファイルがダウンロードできます。
+
* 出力されるTSVの例：xref:enju_webapi_1.adoc#example-bib-tsv[「書誌と所蔵のTSVの例」（Web APIマニ ュアル）]
* 出力される項目の詳細：xref:enju_webapi_1.adoc#bib-tsv-export[書誌と所蔵のTSVの項目（Web APIマニ ュアル）]

=== 検索結果詳細を電子メールで送る

メールアドレスを登録しており、ログインしていると、検索結果詳細を電子メールで送ることができます。

メールアドレスの登録方法はxref:enju_user_3.adoc#profile-info[利用者マニュアル「メールアドレスや名前などを変更する」]を参照してください。

. 電子メールで送りたい検索結果詳細画面を表示させます。[書誌情報を電子メールで送信する]リンクをクリックします
+
image::1.1/search_result_full_mail_1.png[mail]

. [OK]をクリックします。
+
image::1.2/image_alert.png[ok]

. 電子メールで送られてきます。
+
image::1.1/search_result_full_mail_3.png[mail_body]
