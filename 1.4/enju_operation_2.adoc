...
管理者が、一般利用者向けの画面表示を確認したりする場合や、Enjuの機能設定を確認する場合に備えて、ほかの利用者の権限で代理ログインする機能があります。

. ［図書館の管理］メニューから［利用者の管理］を選択します。
+
image::../assets/images/1.1/image_operation_user.png[利用者の管理]
+
. 代理ログインしたい利用者の利用者番号をクリックします。
+
image::../assets/images/1.4/profiles_select.png[利用者の管理]
+
. 右メニューの［ログインユーザを切り替える］をクリックします。
+
image::../assets/images/1.4/profile_impersonate_select.png[ログインユーザを切り替える]
+
. 代理ログインされたことを画面で確認します。表示画面冒頭に「～～としてログイン（代理）」と代理ログイン中であることが表示されます。また、もとの管理者のユーザ権限に戻るリンクも表示されます。
+
image::../assets/images/1.4/profile_impersonated.png[代理ログイン確認]

[NOTE]
====
代理ログイン機能は、当該利用者の権限で扱える機能をすべて実行できてしまうため、運用／利用時のテスト目的でのみとし、実際の機能実行はおこなわないようにすることを薦めます。
====
...
[NOTE]
====
［検索語］にユーザ名や利用者番号を入力して、ユーザを検索することもできます。
====
...
include::../_includes/enju_read_message_admin.adoc[]
