...
にします
+
※ 検索結果詳細画面の右メニューの[TSV]リンクをクリックした先のURLと同様です(詳細：xref:enju_user_1.adoc#bib-tsv-one-record[利用者マニュアル「TSV 形式で書き出し（詳細）」]）。
+
...
xref:enju_webapi_1.adoc#bib-tsv-export[1章の「書誌と所蔵のTSVの項目」]を参照してください。
...
にします
+
※ 検索結果詳細画面の右メニューの[RDF/XML]リンクをクリックした先のURLと同様です(詳細：xref:enju_user_1.adoc#bib-rdf-xml-one-record[利用者マニュアル「RDF/XML 形式で書き出し（詳細）」]）。
+
